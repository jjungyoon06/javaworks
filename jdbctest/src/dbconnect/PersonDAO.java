...
public class PersonDAO {
	...
	public void updatePerson(Person person) {
		conn = JDBCUtil.getConnection();
		String sql = "UPDATE person SET WHERE userpw = ?, name = ?, age = ?, WHERE userId = ?";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, person.getUserPw()); //수정된 자료를 db에 userpw에 저장
			pstmt.setString(0, person.getName());
	...
	public void deletePerson(Person person) {
		conn = JDBCUtil.getConnection();
		String sql = "DELETR FROM person WHERE userid = ?";
		try {
			pstmt = conn.prepareStatement(sql);
	...
	public Person getPerson(String userId) {
		...
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, userId); //입력된 userid 저장
